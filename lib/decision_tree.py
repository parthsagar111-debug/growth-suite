...
principles section). Two corrections applied per direct confirmation:
  ...
  - "Experiment Designer" is also a real page in this same app, so that
    handoff is an in-app st.switch_page, not an external link.
...
def resolve_handoff(tree_id: str, resolved_steps: list, hit_cap: bool) -> dict | None:
    ...
    THIS tree, each carrying a "branch_data" dict (see pages/0_First_Response.py,
    where fr_history entries are built). Walks them back-to-front looking
    for the most recent branch that
    carries a "handoff" key; falls back to the tree's own default_handoff
    only if the session ended by hitting the question cap rather than a
    clean terminal match. Returns None if nothing applies — callers must
    render the explicit non-handoff card in that case, per the cross-tree
    principle of saying so plainly rather than force-fitting one.
    """
    tree = TREES[tree_id]
