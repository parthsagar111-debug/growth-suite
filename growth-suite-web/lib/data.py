...
Ported from the Streamlit app's lib/data.py with one change: every
st.cache_data(ttl=N) is replaced with lib/cache.py's ttl_cache(ttl=N)
(same per-argument caching + a .clear() method), and every st.warning()
call is replaced with logging.warning() — this module has no UI
framework of its own, so surfacing a soft failure is the caller's job
(the FastAPI route can inspect logs or, later, flash a banner). Every
function signature, return shape, and fallback behavior is unchanged.
"""
import os
import json
import logging
...
logger = logging.getLogger("growth_suite.data")
...
# unreachable project can never hang a request indefinitely — supabase-py's
...
def _guarded(fn, default, label):
    """Runs `fn` with a hard timeout; returns `default` (and logs a
    warning) on timeout or any other failure instead of letting an
    unreachable backend freeze the request."""
    try:
        future = _EXECUTOR.submit(fn)
        return future.result(timeout=_SUPABASE_TIMEOUT_SECS)
    except concurrent.futures.TimeoutError:
        logger.warning("%s timed out after %ss — showing demo data instead.", label, _SUPABASE_TIMEOUT_SECS)
        return default
    except Exception as e:
        logger.warning("%s failed (%s) — showing demo data instead.", label, e)
        return default
...
def is_live() -> bool:
    ...
    Client construction happens INSIDE the guarded lambda, so the entire
    operation — not just the query — is bounded by the same 10s cap."""
    if not SUPABASE_URL or not SUPABASE_KEY:
...
def create_brand(name: str, category: str = "Other", discount_stance: str = "discount-light") -> str | None:
    ...
    Validates the name before ever reaching Supabase — the caller should
    already check for a blank name, but a function that writes to the
    database shouldn't rely on callers to have done that correctly."""
    name = (name or "").strip()
    if not SUPABASE_URL or not SUPABASE_KEY or not name:
        return None
    if len(name) > 120:
        logger.warning("Workspace name is too long (120 characters max) — not saved.")
        return None
...
def save_experiment_result(experiment_id: str, lift_pp: float, optout_pp: float,
    ...
    Validates inputs before writing: verdict must be one of the three the
    schema's check constraint allows, and the metrics must actually be
    numbers."""
    if not SUPABASE_URL or not SUPABASE_KEY or not experiment_id:
        return False
    if verdict not in ("SHIP", "KILL", "EXTEND"):
        logger.warning("Invalid verdict '%s' — not saved.", verdict)
        return False
    try:
        lift_pp, optout_pp, support_delta = float(lift_pp), float(optout_pp), int(support_delta)
    except (TypeError, ValueError):
        logger.warning("Grading inputs weren't numeric — not saved.")
        return False
...
def call_workflow(tool: str, payload: dict) -> dict:
    ...
    if url and is_live():
        try:
            ...
            logger.warning("Live workflow call timed out — showing demo data instead.")
        except requests.exceptions.ConnectionError:
            logger.warning("Couldn't reach the live workflow — showing demo data instead.")
        except Exception as e:
            logger.warning("Live workflow call failed (%s) — showing demo data instead.", e)
...
SAMPLE_RESULTS = {
    ...
    "lifecycle_architect": {
        "stages": [
            {"day": 0, "name": "Welcome", "channel": "WhatsApp", "icon": "👋",
            ...
            {"day": 14, "name": "Habit-forming", "channel": "WhatsApp", "icon": "🌱",
            ...
            {"day": 28, "name": "At-risk", "channel": "WhatsApp", "icon": "⚠️",
            ...
            {"day": 45, "name": "Lapsed", "channel": "WhatsApp", "icon": "📭",
            ...
            {"day": 75, "name": "Win-back", "channel": "WhatsApp", "icon": "🎁",
